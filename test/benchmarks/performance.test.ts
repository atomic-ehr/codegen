		...
		it('should generate FHIR types efficiently', async () => {
			...
			const benchmark = await runBenchmark(
				...
				async () => {
					...
					// Since we already have TypeSchemas, we'll simulate the generation
					// In a real scenario, this would use generateFromPackage or generateFromSchemas with FHIR schemas
					return schemas;
				}
			);
