 ...
 */
export function assertFilesEquivalent(actual: GeneratedFile[], expected: GeneratedFile[]): void {
 ...
 */
export function assertPerformanceBenchmark(actualTime: number, baselineTime: number, tolerance: number = 0.1): void {
    ...
    if (propertyMatches) {
        for (const match of propertyMatches) {
            ...
            if (!name.startsWith("_") && !name.includes("_")) {
                // Allow underscore prefixes and snake_case
                expect(name).toMatch(/^[a-z][a-zA-Z0-9]*$/);
            }
        }
    }
    ...
    if (exportMatches) {
        ...
        for (const match of exportMatches) {
            ...
            if (exportLineIndex > 0) {
                ...
                const hasJSDoc =
                    previousLine === "*/" || twoLinesBack?.startsWith("/**") || previousLine?.startsWith("//");
    ...
    if (itemValidator) {
        array.forEach((item, index) => {
            try {
                ...
                throw new Error(
                    `Item validation failed at index ${index}: ${error instanceof Error ? error.message : String(error)}`,
                );
            }
        });
    }
