...
describe("Type Schema generation from FHIR Schema", async () => {
    ...
    it.todo("with resource with string", async () => {
        const fs: PFS = await Bun.file("test/asserts/fhir-schemas/resource-with-string.fs.json").json();
        const ts = await fs2ts(r4, fs);
        expect(ts).toMatchSnapshot();
    });

    it.todo("with resource with code", async () => {
        const fs: PFS = await Bun.file("test/asserts/fhir-schemas/resource-with-code.fs.json").json();
        const ts = await fs2ts(r4, fs);
        expect(ts).toMatchSnapshot();
    });
