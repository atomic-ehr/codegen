...
// run drops the processed cache to make the configuration observable.
const generate = async (opts: object) => {
    const result = await new APIBuilder({ ...opts, logger: mkErrorLogger(), canonicalManager: { dropCache: true } })
