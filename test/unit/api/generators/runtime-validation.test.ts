...
describe('Generated Code Structure Tests', () => {
  test('should generate TypeScript arrays and types correctly', () => {
    ...
    expect(generatedCode).toContain('export type TestBinding = typeof TestBindingValues[number];');
    expect(generatedCode).toContain('export const isValidTestBinding = (value: string): value is TestBinding =>');
    expect(generatedCode).toContain('TestBindingValues.includes(value as TestBinding);');
  });
  ...
  test('should generate proper const assertions and types', () => {
    ...
    expect(generatedCode).toContain('] as const;');
    expect(generatedCode).toContain('typeof ConstAssertionBindingValues[number]');
  ...
  test('should generate proper type guards when helpers enabled', () => {
    ...
    expect(generatedCode).toContain('(value: string): value is TypeGuardBinding');
    expect(generatedCode).toContain('TypeGuardBindingValues.includes(value as TypeGuardBinding)');
  ...
  test('should generate complete syntactically correct TypeScript', () => {
    ...
    expect(generatedCode).toContain('* @see http://hl7.org/fhir/ValueSet/syntax-binding');
    expect(generatedCode).toContain('* @package hl7.fhir.r4.core');
    expect(generatedCode).toContain('* @generated This file is auto-generated. Do not edit manually.');
    expect(generatedCode).toContain('*/');
    expect(generatedCode).toContain('export const SyntaxBindingValues = [');
    expect(generatedCode).toContain('] as const;');
    expect(generatedCode).toContain('export type SyntaxBinding = typeof SyntaxBindingValues[number];');
    expect(generatedCode).toContain('export const isValidSyntaxBinding = (value: string): value is SyntaxBinding =>');
    expect(generatedCode).toContain('SyntaxBindingValues.includes(value as SyntaxBinding);');
  ...
  test('should generate arrays with exact values and length', () => {
    ...
    expect(generatedCode).toMatch(/export const ArrayTestBindingValues = \[\s*'first',\s*'second',\s*'third'\s*\] as const;/);
