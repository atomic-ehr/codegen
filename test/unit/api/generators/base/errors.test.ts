...
describe("Generator Error Classes", () => {
  ...
  describe("SchemaValidationError", () => {
    ...
    test("provides context-aware suggestions", () => {
      ...
      expect(suggestions).toContain(
        "Verify the schema follows the TypeSchema specification",
      );
    ...
    test("provides beginner-friendly suggestions", () => {
      ...
      expect(suggestions.some((s) => s.includes("🎓 Beginner Tips"))).toBe(
        true,
      );
      expect(suggestions.some((s) => s.includes("Quick Start guide"))).toBe(
        true,
      );
      expect(
        suggestions.some((s) =>
          s.includes("Compare with your working schema: Patient"),
        ),
      ).toBe(true);
    });
  ...
  describe("TemplateError", () => {
    ...
    test("handles missing variables", () => {
      ...
      expect(
        suggestions.some((s) => s.includes("Missing template variables")),
      ).toBe(true);
  ...
  describe("FileOperationError", () => {
    ...
    test("provides operation-specific suggestions", () => {
      ...
      expect(
        suggestions.some((s) => s.includes("File writing troubleshooting")),
      ).toBe(true);
      expect(
        suggestions.some((s) => s.includes("output directory exists")),
      ).toBe(true);
      expect(suggestions.some((s) => s.includes("write permissions"))).toBe(
        true,
      );
    });
    ...
    test("provides error-code specific suggestions", () => {
      ...
      expect(suggestions.some((s) => s.includes("📂 File Not Found"))).toBe(
        true,
      );
  ...
  describe("TypeMappingError", () => {
    ...
    test("suggests available and similar mappings", () => {
      ...
      expect(
        suggestions.some((s) => s.includes("Available type mappings")),
      ).toBe(true);
      expect(suggestions.some((s) => s.includes("Suggested mappings"))).toBe(
        true,
      );
    ...
    test("provides language-specific suggestions", () => {
      const tsError = new TypeMappingError(
        "Type error",
        "unknown",
        "TypeScript",
      );
      ...
      expect(
        tsSuggestions.some((s) => s.includes("TYPESCRIPT_PRIMITIVES")),
      ).toBe(true);

      const pyError = new TypeMappingError("Type error", "unknown", "Python");
      const pySuggestions = pyError.getSuggestions();
      expect(pySuggestions.some((s) => s.includes("PYTHON_TYPE_MAP"))).toBe(
        true,
      );
  ...
  describe("BatchOperationError", () => {
    ...
    test("aggregates unique suggestions", () => {
      ...
      expect(
        suggestions.some((s) => s.includes("This is a test suggestion")),
      ).toBe(true);
    ...
    test("provides detailed error breakdown", () => {
      ...
      const errors = [
        new SchemaValidationError("Patient validation failed", schema1, [
          "missing field",
        ]),
        new SchemaValidationError("Observation validation failed", schema2, [
          "invalid type",
        ]),
      ];
    ...
    test("categorizes recoverable and non-recoverable errors", () => {
      ...
      const batchError = new BatchOperationError("Mixed errors", [
        recoverableError,
        nonRecoverableError,
      ]);

      expect(batchError.getRecoverableErrors()).toEqual([recoverableError]);
      expect(batchError.getNonRecoverableErrors()).toEqual([
        nonRecoverableError,
      ]);
