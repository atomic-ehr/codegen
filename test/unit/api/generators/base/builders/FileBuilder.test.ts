...
describe('FileBuilder', () => {
  ...
  describe('Mock FileBuilder Tests', () => {
    ...
    test('manages imports correctly', () => {
      ...
      mockFileBuilder
        .addImport('TestType', './test-type')
        .addImport('AnotherType', './another-type');
...
interface Patient {
  ...
  describe('Performance', () => {
    test('handles large content efficiently', () => {
      const largeContent = 'interface Test {\n' + 
    ...
    test('handles many operations efficiently', () => {
      ...
      for (let i = 0; i < 100; i++) {
        mockBuilder.addImport(`Type${i}`, `./type${i}`)
          .addExport(`Export${i}`);
      }
