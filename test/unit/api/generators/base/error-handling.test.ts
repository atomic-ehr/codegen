...
import { createMockSchema } from "@test/helpers/schema-helpers";
import { MockLogger } from "@test/helpers/mock-generators";
...
describe("Enhanced Error Handling", () => {
    describe("EnhancedSchemaValidationError", () => {
        ...
        test("provides context-aware suggestions", () => {
            const schema = createMockSchema();
            const error = new EnhancedSchemaValidationError("Invalid kind", schema, [
        ...
        test("formats error message nicely", () => {
            ...
            const error = new EnhancedSchemaValidationError("Test validation error", schema, [
                "Field missing",
                "Type invalid",
            ]);
    ...
    describe("ErrorHandler", () => {
        ...
        test("handles single error appropriately", () => {
            const schema = createMockSchema();
            const error = new EnhancedSchemaValidationError("Test error", schema, ["test validation error"]);
        ...
        test("formats errors as JSON when requested", () => {
            ...
            const error = new EnhancedSchemaValidationError("Test error", createMockSchema(), [
    ...
    describe("GeneratorErrorBoundary", () => {
        ...
        test("catches and re-throws errors from async operations", async () => {
            ...
            await expect(boundary.withErrorBoundary(operation)).rejects.toThrow("Test error");
        });
        ...
        test("handles batch operations with mixed results", async () => {
            ...
            await expect(boundary.withBatchErrorBoundary(operations)).rejects.toThrow();
        });
    ...
    describe("Error Context and Suggestions", () => {
        test("provides relevant suggestions based on error type", () => {
            const schemaError = new EnhancedSchemaValidationError("Missing identifier", createMockSchema(), [
                "identifier.name is missing",
            ]);
        ...
        test("includes error context for debugging", () => {
            ...
            const error = new EnhancedSchemaValidationError("Test error", schema, ["test error"]);
