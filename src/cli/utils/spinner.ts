...
export async function withSpinner<T>(
    ...
    try {
        ...
        const successMessage =
            typeof options.success === "function" ? options.success(result) : options.success || options.start;
        spinner.succeed(successMessage);
        return result;
    } catch (error) {
        const failMessage =
            typeof options.fail === "function" ? options.fail(error) : options.fail || `Failed: ${options.start}`;
