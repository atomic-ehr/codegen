...
export const typeschemaCommand: CommandModule = {
	...
	builder: (yargs) => {
		return yargs
			...
			.example("$0 typeschema validate schemas.ndjson", "Validate TypeSchema file")
			.example("$0 typeschema merge schema1.ndjson schema2.ndjson", "Merge multiple TypeSchema files");
	},
	handler: () => {
