...
import { TypeSchemaField, TypeSchemaIdentifier, PackageInfo } from '../types';
import { buildSchemaIdentifier, buildNestedIdentifier, buildBindingIdentifier, dropVersionFromUrl } from './identifier';
 ...
 */
export function mergeElementHierarchy(hierarchy: FHIRSchemaElement[]): FHIRSchemaElement {
  ...
  for (let i = hierarchy.length - 1; i >= 0; i--) {
    ...
    const specificProps = ['choices', 'short', 'index', 'elements', 'required', 'excluded',
                          'binding', 'refers', 'elementReference', 'mustSupport', 
                          'slices', 'slicing', 'url', 'extensions'];
...
export function buildReferences(
  ...
  return element.refers.map(ref => {
    try {
      ...
      if (resource) {
        return buildSchemaIdentifier(resource as unknown as FHIRSchema, packageInfo);
      }
    } catch {
...
export function isNestedElement(element: FHIRSchemaElement): boolean {
  return element.type === 'BackboneElement' || 
         (element.elements && Object.keys(element.elements).length > 0) || false;
}
