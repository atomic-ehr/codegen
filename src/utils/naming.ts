...
export function getReferenceType(targetTypes: string[]): string {
  ...
  const types = targetTypes.map(t => `Reference<${toTypeScriptIdentifier(t)}>`);
  return types.join(' | ');
}
...
export function isChoiceField(fieldName: string): boolean {
  return fieldName.includes('[x]') || getChoiceBaseName(fieldName) !== fieldName;
}
...
export function getModuleName(typeName: string): string {
  ...
  const primitives = [
    ...
    'uuid', 'id', 'oid', 'unsignedInt', 'positiveInt', 'markdown', 'time',
    'date', 'dateTime', 'instant', 'base64Binary', 'code', 'xhtml'
  ];
  ...
  // Complex types
  const complexTypes = [
    'Element', 'BackboneElement', 'Extension', 'Narrative', 'Reference',
    'CodeableConcept', 'Coding', 'Quantity', 'Range', 'Period', 'Ratio',
    'SampledData', 'Identifier', 'HumanName', 'Address', 'ContactPoint',
    'Timing', 'Meta', 'Attachment', 'Duration', 'Distance', 'Count',
    'Money', 'Age', 'SimpleQuantity', 'ContactDetail', 'Contributor',
    'DataRequirement', 'ParameterDefinition', 'RelatedArtifact',
    'TriggerDefinition', 'UsageContext', 'Dosage', 'Population'
  ];
