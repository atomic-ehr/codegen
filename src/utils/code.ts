 ...
 */
export function generateJSDoc(description?: string, tags?: Record<string, string>): string[] {
  const lines: string[] = ['/**'];
...
export function needsImport(typeName: string): boolean {
  ...
  const builtInTypes = [
    ...
    'null', 'undefined', 'object', 'Object', 'Promise'
  ];
  
  // Array is built-in but might be used as Array<T>, so we handle it separately
  if (typeName === 'Array') return false;
  
  return !builtInTypes.includes(typeName) && !typeName.includes('|') && !typeName.includes('<');
}
