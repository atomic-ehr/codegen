...
import { resolvePrimitiveType, tsFieldName, tsGet, tsResourceName } from "./utils";

/**
 * Generate profile interface (current implementation)
 * NOTE: This will be replaced with adapter class in Task 4
 */
export function generateProfileType(writer: Writer, tsIndex: TypeSchemaIndex, flatProfile: ProfileTypeSchema): void {
 ...
 */
export function generateAttachProfile(writer: Writer, flatProfile: ProfileTypeSchema): void {
 ...
 */
export function generateExtractProfile(writer: Writer, tsIndex: TypeSchemaIndex, flatProfile: ProfileTypeSchema): void {
    const tsBaseResourceName = tsResourceName(flatProfile.base);
    const tsProfileName = tsResourceName(flatProfile.identifier);
    ...
    writer.curlyBlock(
        ...
        () => {
            for (const fieldName of profileFields) {
                ...
                if (!isNotChoiceDeclarationField(pField) || !isNotChoiceDeclarationField(rField)) continue;
