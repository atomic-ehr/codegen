...
export class SchemaValidationError extends GeneratorError {
	...
	getSuggestions(): string[] {
		...
		for (const error of this.validationErrors) {
			if (error.includes("identifier.name")) {
				...
				suggestions.push(
					'💡 Example: identifier: { name: "Patient", kind: "resource" }',
				);
			}
			...
			if (error.includes("identifier.kind")) {
				...
				suggestions.push(
					"📚 Check FHIR specification for the correct kind value",
				);
			}
			...
			if (error.includes("circular")) {
				...
				suggestions.push(
					"🔍 Look for schemas that reference each other in a loop",
				);
			}

			if (error.includes("fields")) {
				suggestions.push(
					"📝 Check that all fields have proper type definitions",
				);
				suggestions.push(
					"🔧 Ensure field types reference valid TypeSchema identifiers",
				);
			}
		}
		...
		if (this.userContext?.isBeginnerMode) {
			...
			suggestions.push(
				"📖 Start with the Quick Start guide: docs/getting-started/quick-start.md",
			);
			suggestions.push("🔍 Use --verbose flag for detailed error information");
			suggestions.push(
				"🧪 Test with a simple schema first to verify your setup",
			);
		...
		if (!this.userContext?.isBeginnerMode) {
			...
			suggestions.push(
				"⚡ Consider schema preprocessing if dealing with complex inheritance",
			);
		}
	...
	override isRecoverable(): boolean {
		...
		return this.validationErrors.every(
			(error) => !error.includes("circular") && !error.includes("corruption"),
		);
	}
}
		...
		switch (this.operation) {
			...
			case "write":
				...
				if (this.filePath.includes(" ")) {
					suggestions.push(
						"   • File path contains spaces - check path escaping",
					);
				}
		...
		switch (errorCode) {
			case "EACCES":
				...
				if (this.recoveryOptions?.permissionFix) {
					suggestions.push(
						`   • Fix command: ${this.recoveryOptions.permissionFix}`,
					);
				} else {
			...
			case "ENOSPC":
				...
				if (this.recoveryOptions?.diskSpaceRequired) {
					const mb = Math.ceil(
						this.recoveryOptions.diskSpaceRequired / 1024 / 1024,
					);
			...
			case "ENOENT":
				...
				suggestions.push(
					`   • Create directory: mkdir -p "$(dirname "${this.filePath}")"`,
				);
				break;
...
export class TypeMappingError extends GeneratorError {
	...
	getSuggestions(): string[] {
		...
		suggestions.push(
			`🎯 Type mapping issue for '${this.fhirType}' → ${this.targetLanguage}`,
		);
		...
		if (this.mappingContext?.suggestedMappings) {
			...
			Object.entries(this.mappingContext.suggestedMappings).forEach(
				([fhir, target]) => {
		...
		suggestions.push(
			`   • Add '${this.fhirType}' mapping in ${this.targetLanguage}TypeMapper`,
		);
...
export class ConfigurationError extends GeneratorError {
	...
	getSuggestions(): string[] {
		...
		suggestions.push(
			`   Provided: ${JSON.stringify(this.providedValue)} (${typeof this.providedValue})`,
		);
		...
		suggestions.push(
			`   • Check the '${this.configKey}' value in your configuration`,
		);
...
export class BatchOperationError extends GeneratorError {
	...
	getSuggestions(): string[] {
		...
		const suggestions: string[] = [];
		suggestions.push(
			`📊 Batch operation failed with ${this.errors.length} errors:`,
		);
