...
export class HandlebarsTemplateEngine extends TemplateEngine {
	...
	constructor(options: HandlebarsTemplateEngineOptions) {
		...
		if (this.options.autoLoadTemplates && this.options.templateDirectory) {
			this.loadTemplatesFromDirectory(this.options.templateDirectory).catch(
				(error) => {
					this.logger.warn(
						`Failed to auto-load templates: ${error instanceof Error ? error.message : String(error)}`,
					);
				},
			);
		}
	...
	private setupHandlebars(): void {
		...
		this.handlebars.registerHelper(
			"each_with_index",
			function (this: any, array: any[], options: Handlebars.HelperOptions) {
				let result = "";
		...
		this.handlebars.registerHelper("exports", (exports: Set<string>) =>
			Array.from(exports)
				.map((exp) => `export { ${exp} };`)
				.join("\n"),
		);
	...
	private async discoverTemplateFiles(directory: string): Promise<string[]> {
		...
		try {
			...
			this.logger.warn(
				`Failed to read directory ${directory}: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	...
	private async loadTemplateFile(filePath: string): Promise<void> {
		try {
			...
			this.logger.warn(
				`Failed to load template file ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
