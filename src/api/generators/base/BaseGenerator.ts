...
> {
  ...
  private async validateSchemas(schemas: TypeSchema[]): Promise<void> {
    ...
    const operations = schemas.map(schema => 
      () => this.errorBoundary.withErrorBoundary(
        async () => {
          ...
          this.reportProgress('validation', schemas.indexOf(schema) + 1, schemas.length, 
    ...
    await this.errorBoundary.withBatchErrorBoundary(
      operations,
  ...
  private async generateFiles(schemas: TypeSchema[]): Promise<void> {
    const operations = schemas.map((schema, index) =>
      () => this.errorBoundary.withErrorBoundary(
        async () => {
  ...
  private async generateFileForSchema(
    ...
    // Report progress
    this.reportProgress('writing', index + 1, total, 
