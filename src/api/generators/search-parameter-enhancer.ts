...
export interface SearchModifiers {
	...
	ReferenceParameter<T extends string = string>: 
		| string 
		| { reference: string }
		| { identifier: string }
		| { missing: boolean };
...
export class SearchParameterEnhancer {
	...
	private resourceSearchParams = new Map<string, Array<{
		name: string;
		...
		switch (resourceType) {
			case 'Patient':
				searchParams.push(
					...
					{ name: 'address', type: 'string', description: 'A server defined search that may match any of the string fields in the Address' },
					{ name: 'address-city', type: 'string', description: 'A city specified in an address' },
					{ name: 'address-country', type: 'string', description: 'A country specified in an address' },
					{ name: 'address-postalcode', type: 'string', description: 'A postalCode specified in an address' },
					{ name: 'address-state', type: 'string', description: 'A state specified in an address' },
					{ name: 'address-use', type: 'token', description: 'A use code specified in an address' },
					{ name: 'birthdate', type: 'date', description: 'The patient\'s date of birth' },
					{ name: 'death-date', type: 'date', description: 'The date of death has been provided and satisfies this search value' },
					{ name: 'deceased', type: 'token', description: 'This patient has been marked as deceased, or as a death date entered' },
					{ name: 'email', type: 'token', description: 'A value in an email contact' },
					...
					{ name: 'general-practitioner', type: 'reference', target: ['Organization', 'Practitioner', 'PractitionerRole'], description: 'Patient\'s nominated general practitioner' },
					...
					{ name: 'language', type: 'token', description: 'Language code (irrespective of use value)' },
					{ name: 'link', type: 'reference', target: ['Patient', 'RelatedPerson'], description: 'All patients linked to the given patient' },
					{ name: 'name', type: 'string', description: 'A server defined search that may match any of the string fields in the HumanName' },
					{ name: 'organization', type: 'reference', target: ['Organization'], description: 'The organization that is the custodian of the patient record' },
					{ name: 'phone', type: 'token', description: 'A value in a phone contact' },
			...
			case 'Observation':
				searchParams.push(
					{ name: 'category', type: 'token', description: 'The classification of the type of observation' },
					{ name: 'code', type: 'token', description: 'The code of the observation type' },
					{ name: 'component-code', type: 'token', description: 'The component code of the observation type' },
					{ name: 'component-data-absent-reason', type: 'token', description: 'The reason why the expected value in the element Observation.component.value[x] is missing' },
					{ name: 'component-value-concept', type: 'token', description: 'The value of the component observation, if the value is a CodeableConcept' },
					{ name: 'component-value-quantity', type: 'quantity', description: 'The value of the component observation, if the value is a Quantity, or a SampledData' },
					...
					{ name: 'date', type: 'date', description: 'Obtained date/time. If the obtained element is a period, a date that falls in the period' },
					{ name: 'derived-from', type: 'reference', target: ['DocumentReference', 'ImagingStudy', 'Media', 'QuestionnaireResponse', 'Observation', 'MolecularSequence'], description: 'Related measurements the observation is made from' },
					{ name: 'device', type: 'reference', target: ['Device', 'DeviceMetric'], description: 'The Device that generated the observation data' },
					{ name: 'encounter', type: 'reference', target: ['Encounter'], description: 'Encounter related to the observation' },
					{ name: 'focus', type: 'reference', target: ['Resource'], description: 'The focus of an observation when the focus is not the patient of record' },
					{ name: 'has-member', type: 'reference', target: ['Observation', 'QuestionnaireResponse', 'MolecularSequence'], description: 'Related resource that belongs to the Observation group' },
					{ name: 'identifier', type: 'token', description: 'The unique id for a particular observation' },
					{ name: 'method', type: 'token', description: 'The method used for the observation' },
					{ name: 'part-of', type: 'reference', target: ['MedicationAdministration', 'MedicationDispense', 'MedicationStatement', 'Procedure', 'Immunization', 'ImagingStudy'], description: 'Part of referenced event' },
					{ name: 'patient', type: 'reference', target: ['Patient'], description: 'The subject that the observation is about (if patient)' },
					{ name: 'performer', type: 'reference', target: ['Practitioner', 'PractitionerRole', 'Organization', 'CareTeam', 'Patient', 'RelatedPerson'], description: 'Who performed the observation' },
					{ name: 'specimen', type: 'reference', target: ['Specimen'], description: 'Specimen used for this observation' },
					{ name: 'status', type: 'token', description: 'The status of the observation' },
					{ name: 'subject', type: 'reference', target: ['Patient', 'Group', 'Device', 'Location'], description: 'The subject that the observation is about' },
					{ name: 'value-concept', type: 'token', description: 'The value of the observation, if the value is a CodeableConcept' },
					{ name: 'value-date', type: 'date', description: 'The value of the observation, if the value is a date or period of time' },
					{ name: 'value-quantity', type: 'quantity', description: 'The value of the observation, if the value is a Quantity, or a SampledData' },
					{ name: 'value-string', type: 'string', description: 'The value of the observation, if the value is a string, and also searches in CodeableConcept.text' }
				);
				break;

			case 'Organization':
				searchParams.push(
					{ name: 'active', type: 'token', description: 'Is the Organization record active' },
					{ name: 'address', type: 'string', description: 'A server defined search that may match any of the string fields in the Address' },
					{ name: 'address-city', type: 'string', description: 'A city specified in an address' },
					{ name: 'address-country', type: 'string', description: 'A country specified in an address' },
					{ name: 'address-postalcode', type: 'string', description: 'A postal code specified in an address' },
					{ name: 'address-state', type: 'string', description: 'A state specified in an address' },
					{ name: 'address-use', type: 'token', description: 'A use code specified in an address' },
					{ name: 'endpoint', type: 'reference', target: ['Endpoint'], description: 'Technical endpoints providing access to services operated for the organization' },
					{ name: 'identifier', type: 'token', description: 'Any identifier for the organization (not the accreditation issuer\'s identifier)' },
					{ name: 'name', type: 'string', description: 'A portion of the organization\'s name or alias' },
					{ name: 'partof', type: 'reference', target: ['Organization'], description: 'An organization of which this organization forms a part' },
			...
			case 'Practitioner':
				searchParams.push(
					{ name: 'active', type: 'token', description: 'Whether the practitioner record is active' },
					{ name: 'address', type: 'string', description: 'A server defined search that may match any of the string fields in the Address' },
					{ name: 'address-city', type: 'string', description: 'A city specified in an address' },
					{ name: 'address-country', type: 'string', description: 'A country specified in an address' },
					{ name: 'address-postalcode', type: 'string', description: 'A postal code specified in an address' },
					{ name: 'address-state', type: 'string', description: 'A state specified in an address' },
					{ name: 'address-use', type: 'token', description: 'A use code specified in an address' },
					{ name: 'communication', type: 'token', description: 'One of the languages that the practitioner can communicate with' },
					{ name: 'email', type: 'token', description: 'A value in an email contact' },
					{ name: 'family', type: 'string', description: 'A portion of the family name' },
					{ name: 'gender', type: 'token', description: 'Gender of the practitioner' },
					{ name: 'given', type: 'string', description: 'A portion of the given name' },
					{ name: 'identifier', type: 'token', description: 'A practitioner\'s Identifier' },
					{ name: 'name', type: 'string', description: 'A server defined search that may match any of the string fields in the HumanName' },
					{ name: 'phone', type: 'token', description: 'A value in a phone contact' },
					{ name: 'phonetic', type: 'string', description: 'A portion of either family or given name using some kind of phonetic matching algorithm' },
					{ name: 'telecom', type: 'token', description: 'The value in any kind of contact' }
				);
				break;
			...
			default:
				...
				searchParams.push(
					{ name: 'identifier', type: 'token', description: 'Resource identifier' }
				);
				break;
		}
	}
	...
	private generateResourceSearchInterface(
		...
		if (searchParams.length > 0) {
			...
			for (const param of searchParams) {
				const typeMapping = this.getTypeScriptTypeForSearchParameter(param);
...
${interfaceFields.join('\n')}
	 ...
	 */
	private getTypeScriptTypeForSearchParameter(param: {
		name: string;
		type: 'string' | 'number' | 'date' | 'token' | 'reference' | 'composite' | 'quantity' | 'uri';
		target?: string[];
	}): string {
		switch (param.type) {
			...
			case 'token':
				// Special handling for gender and other known enums
				if (param.name === 'gender') {
					return '\'male\' | \'female\' | \'other\' | \'unknown\' | SearchModifiers[\'TokenParameter\']';
			...
			case 'reference':
				if (param.target && param.target.length > 0) {
					const targetTypes = param.target.map(t => `'${t}'`).join(' | ');
					return `string | SearchModifiers['ReferenceParameter']<${targetTypes}>`;
				}
