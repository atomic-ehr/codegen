...
This captures the required fields for creating a new bodyweight observation. The factory methods use this type, so the compiler enforces that you provide everything the profile demands.
...
`create()` builds both the resource and the wrapper in one call. `createResource()` does the same but returns the plain `Observation` without wrapping it.
