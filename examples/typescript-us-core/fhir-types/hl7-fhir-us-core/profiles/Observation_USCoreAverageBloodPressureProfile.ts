...
export class USCoreAverageBloodPressureProfileProfile {
    ...
    static createResource (args: USCoreAverageBloodPressureProfileProfileParams) : Observation {
        ...
        const componentDefaults = [{"code":{"coding":[{"system":"http://loinc.org","code":"96608-5"}]}},{"code":{"coding":[{"system":"http://loinc.org","code":"96609-3"}]}}] as unknown[]
        const componentWithDefaults = [...(args.component ?? [])] as unknown[]
        if (!componentWithDefaults.some(item => matchesSlice(item, {"code":{"coding":[{"system":"http://loinc.org","code":"96608-5"}]}} as Record<string, unknown>))) componentWithDefaults.push(componentDefaults[0]!)
        if (!componentWithDefaults.some(item => matchesSlice(item, {"code":{"coding":[{"system":"http://loinc.org","code":"96609-3"}]}} as Record<string, unknown>))) componentWithDefaults.push(componentDefaults[1]!)
