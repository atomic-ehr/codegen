...
export class USCoreSmokingStatusProfileProfile {
    ...
    static createResource (args: USCoreSmokingStatusProfileProfileParams) : Observation {
        const categoryDefaults = [{"coding":[{"system":"http://terminology.hl7.org/CodeSystem/observation-category","code":"social-history"}]}] as unknown[]
        const categoryWithDefaults = [...(args.category ?? [])] as unknown[]
        if (!categoryWithDefaults.some(item => matchesSlice(item, {"coding":[{"system":"http://terminology.hl7.org/CodeSystem/observation-category","code":"social-history"}]} as Record<string, unknown>))) categoryWithDefaults.push(categoryDefaults[0]!)
