...
- Package defects are fixed declaratively at two explicit layers: `canonicalManager.patches` repairs raw package data (helpers from `@atomic-ehr/fhir-canonical-manager/patch`), and `typeSchema.excludedCanonicals` drops known-broken canonicals from generation (shipped `builtinExclusions` in `src/typeschema/exclusions.ts` always apply unless `typeSchema.builtinExclusions: false`) — never add ad-hoc workarounds in transformer code
